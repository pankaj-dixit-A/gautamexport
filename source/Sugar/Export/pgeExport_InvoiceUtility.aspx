<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="pgeExport_InvoiceUtility.aspx.cs" Inherits="Foundman_Outword_pgeExport_InvoiceUtility" %>
...
<style type="text/css">
    ...
    <script type="text/javascript">
        ...
        function GetCustomers(pageIndex) {
            ...
            $.ajax({
                type: "POST",
                url: "../Export/pgeExport_InvoiceUtility.aspx/GetCustomers",
        ...
        function OnSuccess(response) {
            ...
            if (customers.length > 0) {
                $.each(customers, function () {
                    var customer = $(this);
                    $("td", row).eq(0).html($(this).find("Inv_No").text());
                    $("td", row).eq(1).html($(this).find("Inv_Date").text());
                    $("td", row).eq(2).html($(this).find("Ac_Address").text());
                    $("td", row).eq(3).html($(this).find("Buyer_Address").text());
                    $("td", row).eq(4).html($(this).find("Qnty_MT").text());
                    $("td", row).eq(5).html($(this).find("Rate_Foregin").text());
                    $("td", row).eq(6).html($(this).find("Amt_Foregin").text());
                    $("td", row).eq(7).html($(this).find("Company_Code").text());
                    $("td", row).eq(8).html($(this).find("Year_Code").text());
            ...
            $('#gvCustomers').on('dblclick', 'tr', function () {
                ...
                if (row_index > 0) {
                    ...
                    if (isNaN(Doc_No)) {
                        ...
                        window.open('../Export/pgeExportInvoice.aspx?Doc_No=' + Doc_No + '&Action=' + Action);
                    }
    ...
    <script type="text/javascript">
        function Accmaster() {
            ...
            var Action = 2;
            var Doc_No = "";
            window.open("../Export/pgeExport_InvoiceUtility.aspx", "_self")
            window.open('../Export/pgeExportInvoice.aspx?Doc_No=' + Doc_No + '&Action=' + Action);
        }
    </script>
    <script type="text/javascript">
            ...
            <div id="popup" style="max-height: 600px; overflow-y: scroll;">
                ...
                <center>
                    ...
                    <asp:GridView ID="gvCustomers" runat="server" AutoGenerateColumns="false" PageSize="25"
                        ...
                        <Columns>
                            <asp:BoundField HeaderStyle-Width="20px" DataField="Inv_No" HeaderText="Doc No"
                                ItemStyle-CssClass="Ac_Code" ControlStyle-BackColor="#ff0066" FooterStyle-BorderWidth="15"
                                FooterStyle-HorizontalAlign="Center" />
                            <asp:BoundField HeaderStyle-Width="50px" DataField="Inv_Date" HeaderText="Doc Date"
                                AccessibleHeaderText="center" />
                            <asp:BoundField HeaderStyle-Width="70px" DataField="Ac_Address" HeaderText="AcCode NAme" />
                            <asp:BoundField HeaderStyle-Width="150px" DataField="Buyer_Address" HeaderText="Buyer Name" />
                            <asp:BoundField HeaderStyle-Width="50px" DataField="Qnty_MT" HeaderText="Qty" />
                             <asp:BoundField HeaderStyle-Width="50px" DataField="Rate_Foregin" HeaderText="Rate" />
                            <asp:BoundField HeaderStyle-Width="50px" DataField="Amt_Foregin" HeaderText="Amount" />
                            ...
                            <%--<asp:BoundField HeaderStyle-Width="50px" DataField="DocDate" HeaderText="DocDate" />--%>
                        </Columns>
                    </asp:GridView>
                </center>
                <br />
